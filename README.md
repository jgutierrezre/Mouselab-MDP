...
- **Plugin** (`jspsych-mouselab-mdp/`) — The core rendering engine: the `MouselabMDP` class, canvas components (State, Edge, Arrow, Text, SplitEdge), utilities, and jsPsych plugin registration. *Extend these files* to add new rendering logic or modify interaction behavior.

- **Trial definition** (`static/json/trials.json`) — A JSON object specifying one MDP: the graph, layout, state/edge display rules, click costs, keys, etc. Generated by `experiment/bin/make_trials.py` or written manually. *Define new trials* to change what the participant sees.

- **Session** (`src/session.coffee` → `static/js/session.js`) — The jsPsych timeline script that orchestrates one or more trial definitions into a complete participant run. It loads trial definitions, assembles the timeline, and calls `jsPsych.init()`. *Modify this file* to change the flow (welcome screen, blocked designs, etc.).
...
## Example
Documentation is sparse, thus it is probably best to learn by example. The code for the demo can be found in [experiment/](/experiment/). Here is an [excerpt](https://github.com/fredcallaway/Mouselab-MDP/blob/master/experiment/src/session.coffee#L57) from the main session file, the definition of a simple Mouselab-MDP trial.
