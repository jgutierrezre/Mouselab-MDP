...
def plot_learning_curve():
    ...
    plt.fill_between(trials,
                     ...
                     [r + s for r, s in zip(rates, sems)],
                     alpha=0.2, color="b")
    plt.axhline(y=0.5625, color="gray", linestyle="--", label="Optimal policy (always A)")
...
def plot_optimal_convergence():
    ...
    trial_step_total = defaultdict(int)
    for r in data:
        trial = int(r["trial_number"])
        step = int(r["decision_step"])
        action = r["action"]
        key = (trial, step)
        by_trial_step[key]["A"] += 1 if action == "A" else 0
        by_trial_step[key]["total"] += 1
    ...
    for i, step in enumerate(steps):
        ...
        for tn in trial_nums:
            key = (tn, step)
            total = by_trial_step[key].get("total", 0)
...
def plot_participant_heatmap():
    """One heatmap per participant: actions over trials."""
    data = read_csv("action_distribution.csv")
    if not data:
        return
